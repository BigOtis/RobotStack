...
- Store ordinary stack settings in `~/.robot-future-stack/config.json` and secrets in `~/.robot-future-stack/secrets.json`.
- Prefer browser-based authentication for `gh` and `gcloud`.
- Before deploying, reuse saved values from `~/.robot-future-stack/config.json` when available.
